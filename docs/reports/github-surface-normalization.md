...
Normalized the **GitHub operational surface** of AIGov Core so the repository presents as an open-source, ledger-authoritative audit runtime (`aigov_audit`). Updated issue and PR templates, discussion templates, `config.yml` contact links, `CODEOWNERS`, composite action description, and operator support doc. No runtime semantics changed.
...
| Asset | Before | After |
|-------|--------|-------|
| `ISSUE_TEMPLATE/config.yml` | Links to `aigov-compliance-engine` | Links to `MonikaDvorackova/aigov-core` |
| Issue templates | Generic “GovAI”; onboarding/hosted framing | **AIGov Core** scope; platform called out as separate |
...
- `.github/ISSUE_TEMPLATE/*` — AIGov Core descriptions, tenant/API key hints, platform exclusion
...
**Do not run blindly.** Review org permissions and current repo settings first (`gh repo view MonikaDvorackova/aigov-core`).

### 1. Repository description and topics

```bash
gh repo edit MonikaDvorackova/aigov-core \
  ...
  --add-topic rust \
  --add-topic python \
  --homepage "https://github.com/MonikaDvorackova/aigov-core#readme"
```
...
```bash
gh repo edit MonikaDvorackova/aigov-core --enable-discussions
```
...
| Category | Purpose |
|----------|---------|
| General | Questions about AIGov Core runtime and docs |
...
1. **“AIGov Core quickstart — evidence → compliance summary → export → verify”** — link `docs/quickstart-runtime.md`
...
```bash
gh api repos/MonikaDvorackova/aigov-core/branches/staging/protection
gh api repos/MonikaDvorackova/aigov-core/branches/main/protection
```

Recommended policy (apply via Settings or `gh api` when ready):

- **`staging`**: require PR, require `govai-ci` / `aigov-core-portable` status, no force push
...
gh api repos/MonikaDvorackova/aigov-core/branches/staging/protection \
  --method PUT \
  -f required_status_checks='{"strict":true,"contexts":["aigov-core-portable"]}' \
...
| **Composite Action** | Tag `v1` / `v1.x` on `aigov-core` (not legacy `aigov-compliance-engine` name) |
...
Upload repository social preview image (Settings → General → Social preview) showing **AIGov Core** + “ledger-authoritative audit runtime”. No Stripe/dashboard imagery.
...
```bash
gh api repos/MonikaDvorackova/aigov-core/milestones -f title="Core runtime hardening" -f state=open
```

### 8. Labels — add (if missing)

```bash
gh label create "runtime" --color "1D76DB" --description "aigov_audit HTTP runtime" -R MonikaDvorackova/aigov-core
gh label create "integrations" --color "0E8A16" --description "Reference integrations and SDK adoption" -R MonikaDvorackova/aigov-core
gh label create "tenant-isolation" --color "B60205" --description "API key tenant mapping / isolation" -R MonikaDvorackova/aigov-core
gh label create "audit-export" --color "5319E7" --description "aigov.audit_export.v1 and verify" -R MonikaDvorackova/aigov-core
gh label create "platform-out-of-scope" --color "FBCA04" --description "GovAI Platform; redirect or close per scope" -R MonikaDvorackova/aigov-core
```
...
```bash
gh label list -R MonikaDvorackova/aigov-core --limit 100
```
...
gh label delete "saas" -R MonikaDvorackova/aigov-core --yes
gh label delete "billing" -R MonikaDvorackova/aigov-core --yes
gh label delete "onboarding" -R MonikaDvorackova/aigov-core --yes
gh label delete "hosted-dashboard" -R MonikaDvorackova/aigov-core --yes
gh label delete "kovali" -R MonikaDvorackova/aigov-core --yes
```

### 10. Enable security features (recommended)

```bash
gh api repos/MonikaDvorackova/aigov-core -X PATCH \
  -f has_issues=true \
  -f has_wiki=false \
  -f has_projects=true
```
