"""Tests for scripts/validate_model_risk_manifest.py."""
...
def _load_mod():
    path = REPO_ROOT / "scripts" / "validate_model_risk_manifest.py"
    spec = importlib.util.spec_from_file_location("validate_model_risk_manifest", path)
...
def test_validate_real_manifest(mod):
    rel = "docs/model-risk/model-risk-manifest.json"
...
def test_validate_missing_manifest(mod, tmp_path: Path):
    payload, code = mod.validate_manifest(tmp_path, "docs/model-risk/model-risk-manifest.json")
    assert code == 1
